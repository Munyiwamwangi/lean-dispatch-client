<template>
  <v-data-table
    :headers="declinedBonusesHeaders"
    :items="declinedBonuses"
  ...
  >
    ...
    <template v-slot:[`item.last_modification_date`]="props">
      {{ $date(props.item.last_modification_date).format('DD-MM-YYYY HH:mm') }}
    </template>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      declinedBonuses: (state) => state.bonus_request.all,
    }),
  },

  created() {
    this.$store.dispatch('bonus_request/retrieveBonusRequests').catch((err) => {
      throw err;
    });
  },
