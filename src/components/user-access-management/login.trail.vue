<template>
  <div>
    <v-navigation-drawer v-model="openCloseFilterDrawer" app right width="320">
      <v-form class="pa-4">
    ...
    <v-container fluid>
      ...
      >
            ...
            <div>
              ...
              >
                ...
                <template #trailing>
                  <v-btn
                    icon
                    @click.stop="openCloseFilterDrawer = !openCloseFilterDrawer"
                  >
        ...
        <!-- login time  -->
        <template v-slot:[`item.loginTime`]="props">
          {{ $date(props.item.loginTime).format('DD-MM-YYYY HH:mm') }}
        </template>
        <!-- logout time -->
        <template v-slot:[`item.logoutTime`]="props">
          <div v-if="checkIfStringBeValidDate(props.item.logoutTime)">
            {{ $date(props.item.logoutTime).format('DD-MM-YYYY HH:mm') }}
          </div>
          <div v-else class="text-capitalize">
            {{ props.item.logoutTime }}
          </div>
        </template>
      </v-data-table>
    </v-container>
...
export default {
  ...
  watch: {
    ...
    filterDateRange() {
      // if (val.length > 1) this.debouncedSearch();
      this.debouncedSearch();
    },
  ...
  methods: {
    ...mapActions('logs', {
      searchLoginTrails: 'searchLoginTrails',
      loadLoginTrail: 'loadLoginTrail',
    }),
    checkIfStringBeValidDate(str) {
    ...
    searchData() {
      ...
      this.searchLoginTrails(payload)
        .then((res) => {
          console.log('search response ', res.data.results);
    ...
    closeNavigationDrawer() {
      this.openCloseFilterDrawer = false;
    },
    ...
    importData() {
      console.info('import data functionality here');
    },
